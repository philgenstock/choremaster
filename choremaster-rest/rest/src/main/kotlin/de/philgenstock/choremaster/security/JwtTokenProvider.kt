package de.philgenstock.choremaster.security

import de.philgenstock.choremaster.user.UserEntity
import de.philgenstock.choremaster.util.Clock
import io.jsonwebtoken.Claims
import io.jsonwebtoken.Jwts
import io.jsonwebtoken.io.Decoders
import io.jsonwebtoken.security.Keys
import org.springframework.stereotype.Component
import java.util.Date
import javax.crypto.SecretKey

@Component
class JwtTokenProvider(
    private val clock: Clock,
) {
    fun generateToken(userEntity: UserEntity): String {
        val today = clock.today()
        val expirationDate = Date(today.time + Companion.jwtExpirationDate)
        val token =
            Jwts
                .builder()
                .subject(userEntity.email)
                .issuedAt(today)
                .expiration(expirationDate)
                .signWith(key())
                .compact()
        return token
    }

    fun validateToken(token: String?): Boolean {
        Jwts
            .parser()
            .verifyWith(key())
            .build()
            .parse(token)
        return true
    }

    fun getUsername(token: String): String {
        val claims =
            Jwts
                .parser()
                .verifyWith(key())
                .build()
                .parse(token)
                .payload as Claims

        return claims.subject
    }

    private fun key(): SecretKey = Keys.hmacShaKeyFor(Decoders.BASE64.decode(jwtSecret))

    companion object {
        private const val jwtExpirationDate: Long = 3600000 // 1h = 3600s and 3600*1000 = 3600000 milliseconds
        private val jwtSecret =
            ""
    }
}
